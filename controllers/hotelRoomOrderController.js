...
const HotelRoomOrderController = {
    createHotelRoomOrder: async (req, res) => {
        try {
            const { room_name, room_price, room_quantity, room_status, room_description, room_thumbnail, user_id } = req.body;
            const HotelRoomCreate = await HotelRoomOrder({
                room_name, room_price, room_quantity, room_status, room_description, room_thumbnail, user_id
            })
